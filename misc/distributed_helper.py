...
def run_process(local_rank_proc, NUM_PROC_PER_SHARD, func, shard_id, NUM_SHARDS, cmd_args, cfg, dist_backend="nccl", proc_init_method="tcp://localhost:9999"):
...
def launch_processes(cmd_args, cfg, func, shard_id, NUM_SHARDS):
    if cfg.NUM_GPUS > 1:
        torch.multiprocessing.spawn(fn=run_process,
                                    nprocs=cfg.NUM_GPUS,
                                    args=(cfg.NUM_GPUS, func, shard_id, NUM_SHARDS, cmd_args, cfg)
                                    )
    else:
        func(cmd_args, cfg)
