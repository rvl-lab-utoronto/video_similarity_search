...
#SBATCH --job-name=kin_noflow_r34
...
srun python $ROOTDIR/online_train.py --iterative_cluster --cfg $ROOTDIR/config/custom_configs/resnet_kinetics_itercluster_cc.yaml --gpu 0,1,2,3 --num_data_workers 4 --batch_size 80 --output $ROOTDIR/output_ucf25-kinetics-iterclus-noflow-multi-r34 --epoch 601 --num_shards 2 --ip_address_port tcp://$MASTER_ADDRESS:$MPORT --compute_canada --checkpoint_path $ROOTDIR/output_ucf25-kinetics-iterclus-noflow-multi-r34/tnet_checkpoints/3dresnet/checkpoint.pth.tar VAL.BATCH_SIZE 80 LOSS.LOCAL_LOCAL_CONTRAST True DATA.INPUT_CHANNEL_NUM 3 ITERCLUSTER.METHOD finch RESNET.MODEL_DEPTH 34
