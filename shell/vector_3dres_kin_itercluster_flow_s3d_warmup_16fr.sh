...
OUTDIR=$ROOTDIR/output_ucf30-kin-s3d-p05-16f-warmup
...
python -u $ROOTDIR/online_train.py --vector --cfg $ROOTDIR/config/custom_configs/resnet_kin_itercluster_flow_vector.yaml --gpu 0,1,2,3 --num_data_workers 4 --batch_size 128 --output $OUTDIR --epoch 1 --ip_address_port tcp://$MASTER_ADDRESS:$MPORT --checkpoint_path $PWD/checkpoint VAL.BATCH_SIZE 40 DATA.SAMPLE_DURATION 16 DATASET.POSITIVE_SAMPLING_P 1.0 DATASET.SAMPLING_STRATEGY random_negative MODEL.ARCH s3d DATA.EVAL_BATCHSIZE_MULTIPLIER 9 DATASET.CLUSTER_PATH $ROOTDIR/kin_tmp_vid_clusters.txt TRAIN.CHECKPOINT_FREQ 0.05
